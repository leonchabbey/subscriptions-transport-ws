...
describe('Client', function () {
    ...
    it('should subscribe once after reconnect', function (done) {
        ...
        wsServer.on('headers', function () {
            if (!isClientReconnected) {
                ...
                var stop_1 = Date.now() + 1100;
                while (Date.now() < stop_1) {
                }
            }
        });
        wsServer.on('connection', function (connection) {
    ...
    it('should get websocket close event as argument on disconnect', function (done) {
        var client = new client_1.SubscriptionClient("ws://localhost:" + TEST_PORT + "/", {
            connectionCallback: function () {
                client.client.close();
            },
        });
        var unregister = client.onDisconnected(function (event) {
            unregister();
